(function () {
  ...
  async function loadEvents() {
    ...
    if (upcomingTarget) {
      upcomingTarget.innerHTML = upcoming.map(renderUpcomingEvent).join('');
    }
    if (pastTarget) {
  ...
  function renderUpcomingEvent(e) {
    ...
    return `
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
        <div class="lg:col-span-7 reveal-up">
          ...
          <p class="font-serif text-lg text-sage mb-8">${escapeHtml(formatLongDate(d).replace(/, \d{4}/, ''))} <span class="text-charcoal/40">@</span> ${escapeHtml(e.time_start || '')}${e.time_end ? ' – ' + escapeHtml(e.time_end) : ''}</p>
          ...
          ${(e.google_calendar_url || e.ical_url || e.outlook365_url || e.outlook_live_url) ? `
          <div class="mt-10">
